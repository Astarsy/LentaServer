...
class UserMessage{
    ...
    public static function create($data){
        ...
        if(isset($data->for_message_id) && $data->for_message_id){
            $data->for_message_id=Utils::clearUInt($data->for_message_id);
            $data->to_user_id=DB::getToUserIdByMsgId($data->for_message_id);
        }elseif(isset($data->to_user_id) && $data->to_user_id){
            $data->to_user_id=Utils::clearUInt($data->to_user_id);
            $data->for_message_id=null;
        }else return null;
    ...
    public function getPost(){
        ...
        return DB::getUserMessage($this->_data->id);
    }
    ...
    public function save(){
        ...
        try{
            ...
            // save new files
            if(!empty($_FILES)){
                $this->_images=new UserImageFiles($this->_pdo,$this->_data->item_of_file,'msgfoto');
                $this->_images->save();
            }
...
INSERT INTO tmp1(id,name) SELECT id,name FROM fotos WHERE id IN(
  SELECT fopi.foto_id FROM fotos_of_message_items fopi
    LEFT JOIN message_items pi ON pi.id=fopi.item_id
...
DELETE FROM message_items WHERE post_id=$pid AND id  NOT IN($existed_iids);");
        $stmt->execute();
    ...
    protected function insert_update(){
        ...
        $sql="
INSERT INTO messages (`id`,`user_id`,`to_user_id`,`for_message_id`,`created_at`,`bgci`,`status`,`access`)
  VALUES ($pid,:ui,:tu,:fm,:now,:bg,:us,:ac)
ON DUPLICATE KEY UPDATE `bgci`=:bg,`updated_at`=NOW(),status=:us,access=:ac";
        ...
        $stmt->bindValue(':tu',$this->_data->to_user_id);
        $stmt->bindValue(':fm',$this->_data->for_message_id);
...
//            if(isset($item->id))throw new \Exception('Unexpected item prop - id');
            ...
            $sql="
INSERT INTO message_items(`id`,`post_id`,`tag`,`text`,`fotos_class`,`fotos_align`)
...
ON DUPLICATE KEY UPDATE `tag`=:ta,`text`=:tx,`fotos_class`=:fc,`fotos_align`=:fa";
        ...
        if(isset($this->_images)){
            foreach($this->_images->fotos as &$foto){
                $sql="
INSERT INTO fotos_of_message_items (`item_id`,`foto_id`,`class`)
VALUES(:ii,:fi,:cl)";
                $stmt=$this->_pdo->prepare($sql);
    ...
    public static function setDeleted($pid){
        ...
        $pdo=DB::getPDO();
        try{
            $sql="UPDATE messages SET deleted_at=NOW() WHERE id=$pid AND user_id=$uid";
            $pdo->query($sql);
            return null;
        }catch(\PDOException $e){
            return $e->getMessage();
...
INSERT INTO tmp_delete (id,name) SELECT f.id,f.name FROM fotos_of_message_items fi
  LEFT JOIN message_items pi ON pi.id=fi.item_id
  LEFT JOIN messages p ON p.id=pi.post_id
